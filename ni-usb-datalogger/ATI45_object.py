...
###################################################################################################
calibration_ati45 = np.matrix('0.15596   0.05364  -0.29616 -22.76401   0.71258  22.77619; '
...
class sensor(object):
    ...
    def read_data(self):
        ...
        self.data_tared = self.data - self.tare_input
        self.data_tared = self.data_tared[np.newaxis, :].T
        self.data_tared_cal = np.dot(self.calibration, self.data_tared)

        self.now = time.time()
        self.time_interval.append(self.now-self.last_time)
